...
/*
        ES 6
        변수 => const : 상수형 변수
                const a = 10;   === > 자동 변수( a=:int)
                뒤에 들어 오는 값에 따라 변환
                a=20;  // ERROR (C언어) : 상수 ==> final
                let : Scope가 명확한 변수 ( 사용 범위가 명확함),(지역 변수)
                function display()
                {
                    let i =10; :: i는 지역변수
                }
                ...
                스프레드 연산자
                const aaa ={ 1,2,3 }
                const bbb = [...aaa] => bbb= [1,2,3]
                배열에 있는 모든 값을 복사 (...a)
                화살표 함수 => function,return를 생략 하고 싶을 경우
                function aa(){
                ...
                => ${} (EL), {문자열},{변수}
                <App4 movie={movie} a={} bbb={}/>
                        ==========  생성자의 매개변수에 값을 전송
                
                props :: 생성자에 들어오는 모든 값을 받음
                props.movie props.aaa props.bbb

                React : 상태관리 프로그램 ( 데이터 관리 프로그램 )
                        === state === :

                class App4 extends Component{

                    constructor(props) => this.props.movie
                    {
                        props,state => 멤버 변수 :: 클래스 영역 전체에서 사용 가능
                        props = > 단점 : 변경을 할 수 없는 변수
                        state > 변경 해서 사용 하는 변수
                        var a = 10;
                        var b ={};
                        var c =1
